...
class AugmentationConfig:
    ...
    # Store configuration as dictionaries for serialization
    transforms_train = A.Compose([
        # A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
        # A.Transpose(p=0.5),
        A.RandomBrightnessContrast(brightness_limit=(-0.1, 0.1), contrast_limit=(-0.1, 0.1), p=0.7),
        A.ShiftScaleRotate(shift_limit=0.1, scale_limit=0.3, rotate_limit=20, border_mode=4, p=0.7),
        A.OneOf([
...
class DataConfig:
    ...
    out_size_zyx: List[int] = field(default_factory=lambda: [196, 196, 196])  # (Z_out, H_out, W_out)
    map_size_zyx: List[int] = field(default_factory=lambda: [25, 25, 25])
...
class ModelConfig:
    ...
    name: str = "aneurysm_3d_v3"
    backbone: str = "resnet18"#"resnet18"# "resnet18"  # "resnet34"
    in_chans: int = 1
    num_classes: int = 14
    drop_rate: float = 0.2
    map_size: List[int] = field(default_factory=lambda: [25, 25, 25])
...
class TrainingConfig:
    ...
    epochs: int = 100
    num_workers: int = 8
    output_dir: str = "/home/tamoto/kaggle/RSNA2025/outputs/model1_3dv3_lossv2_mm90_imgsize196"
